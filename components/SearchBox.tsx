...
import { CountriesMap, I_Country, I_Producer, I_Region, ProducersMap, RegionsMap } from '../redux/interfaces'
...
export function SearchBox() {
    ...
    React.useEffect(() => {
        async function load() {
            ...
            const { array: countries } = await getCollection<I_Country, CountriesMap>('_countries/data', ['name'], 'name')
            const { array: producers } = await getCollection<I_Producer, ProducersMap>('_producers/data', ['name'], 'name')
            const { array: regions } = await getCollection<I_Region, RegionsMap>('_regions/data', ['name', 'country', 'countryId'], 'name')
    ...
    async function changeQuery(queryString: string) {
        await router.push(`/stickers/${queryString}`,
            undefined,
            { shallow: true }
        )
    }
        ...
        <div className='search-container'>
            ...
            {/* <div className='mb-2'>
                <InlineComboBox
                    ...
                    items={searchTypes.map(({ value, displayText }) => {
                        return { value, displayText }
                    })}
                    handleChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                    firstOption={'Select search type'}
            ...
            {searchType === 'producer' && (
                <ComboBoxWithButton
                    ...
                    handleChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                    onClickHandler={() => changeQuery(`?queryType=searchByProducer&producerId=${formik.values.producer}`)}
                />
            )}
            ...
            {searchType === 'sku' && (
                ...
                <InputWithButton
                    ...
                    handleChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                    onClickHandler={() => changeQuery(`?queryType=searchBySKU&sku=${formik.values.sku}`)}
                />
