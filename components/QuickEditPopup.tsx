...
export function QuickEditPopup({ closeHandler }: Props) {
  ...
  const quickEditSticker = useSelector(
    (state: StoreState) => state.stickers.quickEditSticker
  );
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      harvestYear: Yup.number()
        .required("Required")
        .moreThan(1989, "year must be more than 1990")
