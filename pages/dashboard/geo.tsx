...
export default function DashboardGeo() {
  ...
  React.useEffect(() => {
    ...
    async function load() {
      unsubscribeCountries = fbInstance.db
        .doc("_countries/data")
        .onSnapshot((doc) => {
          setLoading(true)
      ...
      unsubscribeRegions = fbInstance.db
        .doc("_regions/data")
        .onSnapshot((doc) => {
          setLoading(true)
      ...
      unsubscribeAppellations = fbInstance.db
        .doc("_appellations/data")
        .onSnapshot((doc) => {
          setLoading(true)
  ...
  async function addCountry() {
    ...
    await fbInstance.db
      .doc("_countries/data")
      .update({ [id]: { name: formik.values.newCountryName } })
  ...
  async function deleteCountry(countryId: string) {
    ...
    if (confirmDelete) {
      await fbInstance.db
        .doc("_countries/data")
        .update({ [countryId]: firebase.firestore.FieldValue.delete() })
      setSelectedCountry("")
      setSelectedRegion("")
    }
  }
  ...
  async function addRegion() {
    ...
    const newRegion = {
      name: formik.values.newRegionName,
      countryId: selectedCountry,
      country: countriesMap[selectedCountry].name,
    }
    await fbInstance.db.doc("_regions/data").update({ [id]: newRegion })
  ...
  async function deleteRegion(regionId: string) {
    ...
    if (confirmDelete) {
      await fbInstance.db
        .doc("_regions/data")
        .update({ [regionId]: firebase.firestore.FieldValue.delete() })
      setSelectedRegion("")
    }
  }
  async function addAppellation() {
    const id = fbInstance.db.collection("_appellations").doc().id
    const newAppellation = {
      name: formik.values.newAppellationName,
      regionId: selectedRegion,
      region: regionsMap[selectedRegion].name,
    }
    await fbInstance.db
      .doc("_appellations/data")
  ...
  async function deleteAppellation(appellationId: string) {
    ...
    if (confirmDelete) {
      await fbInstance.db
        .doc("_appellations/data")
        .update({ [appellationId]: firebase.firestore.FieldValue.delete() })
    }
  }
  ...
  return (
    <Layout title={"Dashborad"}>
      ...
      <div className="geo">
          ...
          <div className="d-flex py-2">
            <div className="w-75">
              <Input
                ...
                error={
                  formik.touched.newCountryName && formik.errors.newCountryName
                }
            ...
            <div className="ml-2 w-25">
              <button
                ...
                onClick={addCountry}
                className={
                  "btn btn-info w-100 h-100 font-weight-bold text-no-wrap"
                }
              >
                Add country
              </button>
          ...
          <div className="list hide-scrollbar">
            {countries.map((c) => (
              <DashboardListItem
                ...
                showEditPopup={() =>
                  initEditPopup(c, "_countries/data", "Edit country name")
                }
              />
            ))}
          </div>
            ...
            <div className="d-flex py-2">
              <div className="w-75">
                <Input
                  ...
                  error={
                    formik.touched.newRegionName && formik.errors.newRegionName
                  }
              ...
              <div className="ml-2 w-25">
                <button
                  onClick={addRegion}
                  type="button"
                  className="btn btn-info w-100 h-100 font-weight-bold text-no-wrap"
                >
                  Add region
                </button>
              </div>
          ...
          <div className="list hide-scrollbar">
            {filteredRegions.map((r) => (
              <DashboardListItem
                ...
                showEditPopup={() =>
                  initEditPopup(r, "_regions/data", "Edit region name")
                }
              />
            ))}
          </div>
        ...
        <div className="geo-appellations">
          {selectedRegion && (
            <div className="d-flex py-2">
              <div className="w-75">
                <Input
                  ...
                  error={
                    formik.touched.newAppellationName &&
                    formik.errors.newAppellationName
                  }
