...
import { AppellationsMap, CountriesMap, GrapesMap, I_Appellation, I_Country, I_Grape, I_Producer, I_Region, ProducersMap, RegionsMap, StickerType } from '../redux/interfaces';
...
export default function Create() {
    ...
    const [popup, setPopup] = React.useState<{ showPopup: boolean, msg: string }>({ showPopup: false, msg: '' })
    ...
    async function saveSticker(values: FormikValuesType) {
        ...
        if (router.query.editStickerId) {
            await fbInstance.db.doc(`_stickers/${router.query.editStickerId}`).update(getStickerObject(values))
    ...
    function getStickerObject(values: FormikValuesType): StickerType {
        ...
        const _producer: I_Producer = {
            ...
            producerFullData: subjects.producers.map[values.producer].producerFullData
    ...
    React.useEffect(() => {
        async function loadData() {
            setLoading(true)
            const { array: countries, map: countriesDoc } = await getCollection<I_Country, CountriesMap>('_countries/data', ['name'], 'name')
            const { array: producers, map: producersDoc } = await getCollection<I_Producer, ProducersMap>('_producers/data', ['name', 'producerFullData'], 'name')
            const { array: grapes, map: grapesDoc } = await getCollection<I_Grape, GrapesMap>('_grapes/data', ['name'], 'name')
            const { array: regions, map: regionsDoc } = await getCollection<I_Region, RegionsMap>('_regions/data', ['name', 'country', 'countryId'], 'name')
            const { array: appellations, map: appellationDoc } = await getCollection<I_Appellation, AppellationsMap>('_appellations/data', ['name', 'region', 'regionId'], 'name')
        ...
        async function loadSticker(grapes: Array<I_Grape>) {
            const editSticker = await fbInstance.db.doc(`_stickers/${router.query.editStickerId}`)
                .get()
    ...
    //handle pick grape
    React.useEffect(() => {
        if (!formik.values.pickedGrape)
            return
        debugger
        const selectedGrape: I_Grape = { id: formik.values.pickedGrape, name: subjects.grapes.map[formik.values.pickedGrape].name }
    ...
    function removeGrapeFromList(grapeId: string) {
        ...
        const grape: I_Grape = { id: grapeId, name: subjects.grapes.map[grapeId].name }
        debugger
        grapes.push(grape)
                ...
                <form className='form-container' onSubmit={formik.handleSubmit}>
                    <div className='original-title'>
                        <InlineInput
                            ...
                            error={formik.touched.originalTitle && formik.errors.originalTitle}
                        />
                    ...
                    <div className='create-btn'>
                        <button disabled={selectedGrapes.length === 0} type='submit' className={'btn btn-info w-100'}>
                    ...
                    <div className='clear-form-btn'>
                        <button type='button' onClick={clearForm} className={'btn btn-warning w-100'}>Clear form</button>
                    </div>
                    ...
                    <div className='basic-parameters'>
                        ...
                        <InlineInput
                            ...
                            error={formik.touched.shelfLifetime && formik.errors.shelfLifetime}
                        />
                        <InlineInput
                            inputAttributes={{
                            ...
                            error={formik.touched.servingTemperature && formik.errors.servingTemperature}
                        />
                        <InlineInput
                            inputAttributes={{
                                name: 'harvestYear',
                    ...
                    <div className='region-parameters'>
                        ...
                        <div className='mb-1'>
                            <InlineComboBox
                                ...
                                labelWidth={120}
                                error={formik.touched.regionControl && formik.errors.regionControl}
                            />
                        </div>
                        ...
                        {formik.values.country &&
                            <div className='mb-1'>
                                {filteredRegions.length > 0 && <InlineComboBox
                                    label={'Region'}
                                    name={'region'}
                        ...
                        {formik.values.region &&
                            <div className='mb-1'>
                                {filteredAppellations.length > 0 && <InlineComboBox
                                    label={`Appellation`}
                                    name={'appellation'}
                    ...
                    <div className='grapes-data'>
                        ...
                        {selectedGrapes?.length > 0 && <div className='font-weight-bold'>selected grapes:</div>}

                        {subjects.grapes.array.length > 0 && selectedGrapes?.length > 0 &&

                            selectedGrapes.map((grape) => {
                                return <SelectedGrapeBadge
                                    key={grape.id}
                                    grape={grape}
